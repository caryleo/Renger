...
void CAR::turn(double deg)
{
	...
	double seta = acos(-car_point.dirz / sqrt(car_point.dirx * car_point.dirx + car_point.dirz * car_point.dirz));
	//计算模型应该偏转的角度，这里认为模型默认朝向-z
	if (car_point.dirx > 0)
	{
...
void CAR::update() //更新汽车运动状态
{
	...
	if (tmpIndex != -1)
	{
		...
		double b = wall[tmpIndex].dirx * wall[tmpIndex].dirx + wall[tmpIndex].dirz * wall[tmpIndex].dirz + car_point.
			dirx * car_point.dirx + car_point.dirz * car_point.dirz;
		...
		else
		{
			...
			//计算方向投影
			double tmpx = (car_point.dirx * wall[tmpIndex].dirx + car_point.dirz * wall[tmpIndex].dirz) / (wall[tmpIndex
			].dirx * wall[tmpIndex].dirx + wall[tmpIndex].dirz * wall[tmpIndex].dirz) * wall[tmpIndex].dirx;
			double tmpz = (car_point.dirx * wall[tmpIndex].dirx + car_point.dirz * wall[tmpIndex].dirz) / (wall[tmpIndex
			].dirx * wall[tmpIndex].dirx + wall[tmpIndex].dirz * wall[tmpIndex].dirz) * wall[tmpIndex].dirz;
			...
			//调整模型方向
			double seta = acos(
				-car_point.dirz / sqrt(car_point.dirx * car_point.dirx + car_point.dirz * car_point.dirz));
			//计算模型应该偏转的角度，这里认为模型默认朝向-z
			if (car_point.dirx > 0)
			{
				gothicTrans_car[6] = seta / PI * 180;
...
{
	AABB tmp = AABB(min(m_Xmin,m_Xmax),min(m_Ymin,m_Ymax),min(m_Zmin,m_Zmax),max(m_Xmin,m_Xmax),max(m_Ymin,m_Ymax),
	                max(m_Zmin,m_Zmax));
	tmp.wallInit();
	wall.push_back(tmp);
}
