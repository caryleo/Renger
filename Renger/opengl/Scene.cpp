...
bool CProtechny::Init(int num)
{
	...
	if(CParticle::Init(num))
	{
		for(int i=0; i<m_iNum; i++)
		{
			...
			/** 初始化加速度 */
			m_pList[i].acceleration = CVector966(0.0,-0.00025f,0.0);
...
void CProtechny::Update()
{
	for(int i = 0; i<m_iNum; ++i)
	{
		...
		if (m_pList[i].position.y <= -1 || m_pList[i].lifetime <= 0)
		{
			...
			/** 初始化速度 */
			vx = 0.0000014f * (rand()%9000-rand()%9000);
			vy = 0.0000084f * (rand()%9000);
			vz = 0.0000002f * (rand()%90000);
			m_pList[i].velocity = CVector966(vx,vy,vz);
...
void CScene::init()
{
	...
	/** 初始化雪花实例 */	
	if(!m_Snow->Init(10000))
	{
		MessageBox(NULL,"雪花系统初始化失败!","错误",MB_OK);
		exit(-1);
	}

	if (!m_Protechny->Init(10000))
	{
		MessageBox(NULL,"雪花系统初始化失败!","错误",MB_OK);
		exit(-1);
	}
